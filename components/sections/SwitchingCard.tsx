...
const Leader = [
    ...
    // Name optimal di maksimal 40 karakter
    {
        ...
        message: "Kamu tau ga sih apa bedanya kamu sama matahari? Gaada bedanya sama sama cantik soalnya. Kamu tahu nggak, aku seperti mentega, dan kamu seperti wajan panas. Soalnya pas lihat muka kamu, aku jadi meleleh. Sayangku tadi pagi aku gak bisa makan karena merindukanmu. anjay gg gaming gacor king waw sangat keren sekali damn huehehhh hahahhah" 
    },
    {
    ...
    return (
        <div className="flex flex-col gap-8 py-36">
            <div className="flex justify-center">
                <div className="text-white font-sfPro text-center text-xl sm:text-2xl lg:text-4xl xl:text-5xl">
                    Messages from Class Leader
                </div>
            </div>

            <div className="w-full flex justify-center">
                <div className={`w-[95%] flex gap-7 ${(isMediumScreen || isSmallScreen) ? 'flex-col' : 'justify-center'}`}>
                    {Leader.map((leader, index) => (
                        <div
                            key={index}
                            onClick={() => handleClick(index)}
                            className={`cursor-pointer rounded-[40px] p-[2px] bg-gradient-to-br from-gray-200 via-gray-400 to-gray-600 transition-all duration-500 ease-in-out overflow-hidden ${
                                isSmallScreen
                                    ? 'max-w-[350px]'
                                    : isMediumScreen
                                    ? 'max-w-[500px]' 
                                    : activeCard === index ? 'max-w-[740px]' : 'max-w-[300px]'
                            } ${(isSmallScreen || isMediumScreen) && index === 1 ? 'self-end' : ''}`}
                        >
                            <div className={`w-fit ${(isMediumScreen || isSmallScreen) ? 'py-[40px] px-[15px]' : 'py-[81px] px-[30px]'} ${
                                isSmallScreen || isMediumScreen ? 'max-h-[220px]' : 'max-h-[350px]'
                                ...
                                (isSmallScreen || isMediumScreen || activeCard === index) ? 'gap-6' : 'gap-0'
                            }`}>
                                <Image
                                    src={leader.image}
                                ...
                                <div
                                    className={`text-justify font-[400] ${isSmallScreen || isMediumScreen ? 'text-[10px]' : 'text-base'} text-white font-sfSemi transition-all duration-500 ease-in-out ${
                                        (isSmallScreen || isMediumScreen || activeCard === index) ? 'opacity-100 w-[80%] max-md:w-[60%] max-sm:w-[70%]' : 'opacity-0 w-0'
                                    }`}
                                >
                                    <p
                                    ...
                                    >
                                        {leader.message}<br/> <br/> <span className="font-sf font-[650]">{leader.name}</span>
                                    </p>
                                </div>
                            </div>
                        </div>
