    ...
    return (
        <>
            <div className="w-[186px] h-[237px] max-md:w-[134px] max-md:h-[171px] bg-[#292929] rounded-[14.2px] customShadow relative overflow-hidden z-0 group">
                ...
                <div className="w-fit text-[#b3b3b3] text-center tracking-widest font-[400] font-UncialAntiqua flex flex-col gap-20 items-center z-[2] absolute top-[18px] left-1/2 -translate-x-1/2 opacity-70">
                    <p
                        className={`transition-opacity duration-500 group-hover:opacity-0 select-none ${isSmallScreen ? "leading-[30px]" : "leading-[37px]"}`}
                        style={{ fontSize: isSmallScreen ? data.fontNickname[1] : data.fontNickname[0] }}
                    >
                        {data.nickname}<br />{data.nickname}<br />{data.nickname}<br />{data.nickname}
                    </p>
                    <p
                        className={`absolute top-0 transition-opacity duration-500 opacity-0 group-hover:opacity-100 select-none ${isSmallScreen ? "leading-[30px]" : "leading-[37px]"}`}
                        style={{ fontSize: isSmallScreen ? data.fontDivisi[1] : data.fontDivisi[0] }}
                    >
                        {data.Divisi}<br />{data.Divisi}<br />{data.Divisi}<br />{data.Divisi}<br />
                    </p>
                </div>
