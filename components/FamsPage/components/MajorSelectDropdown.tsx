    ...
    <div className="flex-auto flex justify-end max-w-fit max-md:w-fit">
      {majorTagName ? (
        ...
        <button
          ...
          className="relative bottom-3 right-6"
        >
          <ChevronDown
            className={`${dropdownOpen ? `scale-0` : `scale-100`}  transition-all absolute p-auto max-md:w-[20px]`}
          />
          <ChevronUp
            className={`${dropdownOpen ? `scale-100` : `scale-0`} transition-all absolute p-auto max-md:w-[20px]`}
          />
        </button>
      )}
      ...
      <div className="relative">
        <div
          className={`${dropdownOpen ? "scale-100" : "scale-0"} transition-all absolute top-8 right-0 
